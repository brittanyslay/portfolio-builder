...
## Fast path - LinkedIn + résumé to a near-final draft (the common case)
The target is a **distinct-looking portfolio that needs very minimal edits, built from a LinkedIn profile + a résumé, fast** - not a from-scratch interrogation. Those two documents are almost everything you need; use them.
1. **Ingest both and auto-build the dossier** (Phase 1). LinkedIn + résumé cover titles, dates, employers, scope, and most metrics - reconcile them against each other and flag only the gaps and any number that needs a defensibility check.
2. **Derive everything derivable, don't ask for it**: positioning from the material, the **palette from their profile photo** (Phase 4 default), the **font pairing from their vibe**, employer logos via automated sourcing.
3. **Ask only what you genuinely cannot derive** - a handful of things, not a questionnaire: the **goal + primary CTA** and target roles/JDs, any **discretion constraints** (quiet search, sensitive employer), the **direction / mood** questions (not colors - see `references/design-guidance.md`), and a quick **confirm on the photo**. Everything factual comes from the two documents.
4. **Produce a full draft, then ONE critique + honesty pass**, and hand it over. The honesty gate still holds - every claim traces to the résumé/LinkedIn or gets confirmed - but a truthful, well-positioned, distinct-looking draft belongs in front of the person quickly, and each build looks different because its palette, type, and accent are derived from *this* person (see Phase 4's "do not let every build look the same").
...
- **Color and font are the only open choices, and neither is a "pick a color" exercise.** For color, **default to a portrait-derived palette**: sample the person's profile photo and build the palette from it - their real colors, or a calibrated variation that harmonizes with them (`references/templates/palettes.md`). This is the anti-slop move AND it kills the biggest rework loop, because you never ask a person to name colors they can't judge; you ask them for *direction and mood* (`references/design-guidance.md`) and derive the hex yourself. Present the derived palette (plus maybe one alternate direction) applied to the real hero for a yes/tweak, not a blank color choice. Fall back to a curated preset only when there's no usable photo. For font, pick a pairing from `references/templates/fonts.md` that fits the person's read and show it on the real hero.
- **Do not let every build look the same.** The template skeleton is shared on purpose, but two portfolios off it must not look like siblings. Three levers make each one distinct without touching the proven layout: the **palette is portrait-derived** (unique to the person by construction), the **font pairing is chosen to their vibe** (not always the default), and the **hero/accent treatment is varied** (portrait crop and shape, which module leads, the eyebrow and gradient-accent styling). Rotate these deliberately - if the last portfolio you built looks like this one, change the font pairing and accent treatment until it doesn't.
...
- **Résumé:** default to the locked résumé template (`references/templates/resume-template.html`) - a tokenized, print/PDF US-Letter résumé with its own locked type system (one font, a five-size scale, strong section breaks, ruthless widow/orphan control, group integrity, disciplined bold; the seven rules are documented in the template header and in `references/design-guidance.md`). Fill the `{{PLACEHOLDER}}` tokens from the dossier, produce a branded version + an **ATS-safe** variant, 2 pages, PDF via headless print. Verify no widows/orphans in a real browser before shipping. Keep the template tokenized: never commit a real person's filled résumé to a shared/public repo - fill locally, deliver the PDF.
